...
The heart of the project lies in the implementation of two distinct models. The LSTM model, known for its ability to capture temporal dependencies, is employed to analyze sequential data, while a Random Regressor model adds a different perspective, leveraging a tree-like model of decisions for robust predictions. LSTM1.py contains the LSTM model and randomforestregressor.py contains the Random Regressor Model.

The model evaluation is already integrated within the training of the MLMs (LSTM1.py and randomforestregressor.py), so for the LSTM model I went with MSE (Mean Squared Error), RMSE (Root Mean Squared Error), MAE (Mean Absolute Error) and MAPE (Mean Absolute Percentage Error). For the Random Regressor model I went with MSE, RMSE, MAE and R2 (Coefficient of Determination).
...
For the Random Regressor model is much easier, playing around with the test_size is enough, then the other variable is n_estimators, which represents the number of trees in the forest, the higher the number the better the model, but the more time and computational power you need.
...
Implementing Long Short-Term Memory (LSTM) and Random Regressor models provided insights into handling sequential data and transparent decision-making processes. The LSTM's ability to capture long-term dependencies and the Random Regressor's interpretability were key takeaways.
